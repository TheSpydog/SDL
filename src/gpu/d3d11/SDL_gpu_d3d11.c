    ...
    SDL_GPUColorAttachmentDescription *colorAttachments)
{
    ...
    for (Uint32 i = 0; i < numColorAttachments; i += 1) {
        blendDesc.RenderTarget[i].BlendEnable = colorAttachments[i].blend_state.blend_enable;
        ...
        blendDesc.RenderTarget[i].DestBlend = SDLToD3D11_BlendFactor[colorAttachments[i].blend_state.dst_color_blend_factor];
        blendDesc.RenderTarget[i].DestBlendAlpha = SDLToD3D11_BlendFactorAlpha[colorAttachments[i].blend_state.dst_alpha_blend_factor];
        blendDesc.RenderTarget[i].RenderTargetWriteMask = colorAttachments[i].blend_state.color_write_mask;
        blendDesc.RenderTarget[i].SrcBlend = SDLToD3D11_BlendFactor[colorAttachments[i].blend_state.src_color_blend_factor];
        blendDesc.RenderTarget[i].SrcBlendAlpha = SDLToD3D11_BlendFactorAlpha[colorAttachments[i].blend_state.src_alpha_blend_factor];
    }
    ...
    SDL_GPUDepthStencilState depth_stencil_state)
{
     ...
     * The spec says the driver will not create duplicate states, so there's no need to cache.
     */
    dsDesc.DepthEnable = depth_stencil_state.depth_test_enable;
    dsDesc.StencilEnable = depth_stencil_state.stencil_test_enable;
    dsDesc.DepthFunc = SDLToD3D11_CompareOp[depth_stencil_state.compare_op];
    dsDesc.DepthWriteMask = (depth_stencil_state.depth_write_enable ? D3D11_DEPTH_WRITE_MASK_ALL : D3D11_DEPTH_WRITE_MASK_ZERO);
    ...
    SDL_GPURasterizerState rasterizer_state)
{
    ...
    rasterizerDesc.FrontCounterClockwise = (rasterizer_state.front_face == SDL_GPU_FRONTFACE_COUNTER_CLOCKWISE);
    ...
    size_t shaderByteLength)
{
    ...
    if (inputState.vertex_attribute_count == 0) {
        return NULL;
    }
    ...
    elementDescs = SDL_stack_alloc(
        ...
        inputState.vertex_attribute_count);

    // Create the array of input elements
    for (Uint32 i = 0; i < inputState.vertex_attribute_count; i += 1) {
        ...
        bindingIndex = D3D11_INTERNAL_FindIndexOfVertexBinding(
            ...
            inputState.vertex_binding_count);
    ...
    const SDL_GPUComputePipelineCreateInfo *createinfo)
{
    ...
    pipeline->readonly_storage_texture_count = createinfo->readonly_storage_texture_count;
    pipeline->writeonly_storage_texture_count = createinfo->writeonly_storage_texture_count;
    pipeline->readonly_storage_buffer_count = createinfo->readonly_storage_buffer_count;
    pipeline->writeonly_storage_buffer_count = createinfo->writeonly_storage_buffer_count;
    pipeline->uniform_buffer_count = createinfo->uniform_buffer_count;
    ...
    const SDL_GPUGraphicsPipelineCreateInfo *createinfo)
{
    ...
    pipeline->numColorAttachments = createinfo->attachment_info.color_attachment_count;
    ...
    if (createinfo->vertex_input_state.vertex_binding_count > 0) {
        pipeline->vertexStrides = SDL_malloc(
            sizeof(Uint32) *
            createinfo->vertex_input_state.vertex_binding_count);
    ...
    const SDL_GPUSamplerCreateInfo *createinfo)
{
    ...
    samplerDesc.ComparisonFunc = (createinfo->compare_enable ? SDLToD3D11_CompareOp[createinfo->compare_op] : SDLToD3D11_CompareOp[SDL_GPU_COMPAREOP_ALWAYS]);
    samplerDesc.MaxAnisotropy = (createinfo->anisotropy_enable ? (UINT)createinfo->max_anisotropy : 0);
    ...
    D3D11_SUBRESOURCE_DATA *initialData)
{
    ...
    if (createInfo->type != SDL_GPU_TEXTURETYPE_3D) {
        ...
        desc2D.MipLevels = createInfo->level_count;
        desc2D.MiscFlags = 0;
        ...
        desc3D.MipLevels = createInfo->level_count;
    ...
    d3d11Texture->subresourceCount = createInfo->level_count * layerCount;
    ...
    bool cycle)
{
    ...
    Uint32 bufferStride = source->image_pitch;
    Uint32 bufferImageHeight = source->image_height;
    ...
    stagingTextureCreateInfo.level_count = 1;
    ...
    const SDL_GPUTextureTransferInfo *destination)
{
    ...
    Uint32 bufferStride = destination->image_pitch;
    Uint32 bufferImageHeight = destination->image_height;
    ...
    const SDL_GPUDepthStencilAttachmentInfo *depth_stencil_attachment_info)
{
    ...
    ID3D11DeviceContext_OMSetRenderTargets(
        ...
        color_attachment_count > 0 ? rtvs : NULL,
        dsv);
    ...
    Uint32 binding_count)
{
    ...
    d3d11CommandBuffer->vertexBufferCount =
        SDL_max(d3d11CommandBuffer->vertexBufferCount, first_binding + binding_count);
    ...
    Uint32 storage_buffer_binding_count)
{
    ...
    for (i = 0; i < storage_buffer_binding_count; i += 1) {
        ...
        d3d11CommandBuffer->computeUnorderedAccessViews[i + storage_texture_binding_count] = buffer->uav;
    }
    ...
    Uint32 binding_count)
{
    ...
    for (i = 0; i < binding_count; i += 1) {
        ...
        d3d11CommandBuffer->computeShaderResourceViews[first_slot + i +
                                                       d3d11CommandBuffer->compute_pipeline->readonly_storage_texture_count] = bufferContainer->activeBuffer->srv;
    }
    ...
    D3D11CommandBuffer *command_buffer)
{
    ...
    Uint32 writeOnlyResourceCount =
        compute_pipeline->writeonly_storage_texture_count +
        compute_pipeline->writeonly_storage_buffer_count;
    ...
    if (command_buffer->needComputeUniformBufferBind) {
        for (i = 0; i < compute_pipeline->uniform_buffer_count; i += 1) {
            /* stupid workaround for god awful D3D11 drivers
             * see: https://learn.microsoft.com/en-us/windows/win32/api/d3d11_1/nf-d3d11_1-id3d11devicecontext1-vssetconstantbuffers1#calling-vssetconstantbuffers1-with-command-list-emulation
             */
    ...
    SDL_GPUPresentMode present_mode)
{
    ...
    windowData->textureContainer.header.info.level_count = 1;
    ...
    D3D11Renderer *renderer)
{
    ...
    SDL_zero(createinfo);
    createinfo.code = (Uint8 *)D3D11_FullscreenVert;
    createinfo.code_size = sizeof(D3D11_FullscreenVert);
    ...
    createinfo.code = (Uint8 *)D3D11_BlitFrom2D;
    createinfo.code_size = sizeof(D3D11_BlitFrom2D);
    ...
    createinfo.code = (Uint8 *)D3D11_BlitFrom2DArray;
    createinfo.code_size = sizeof(D3D11_BlitFrom2DArray);
    ...
    createinfo.code = (Uint8 *)D3D11_BlitFrom3D;
    createinfo.code_size = sizeof(D3D11_BlitFrom3D);
    ...
    createinfo.code = (Uint8 *)D3D11_BlitFromCube;
    createinfo.code_size = sizeof(D3D11_BlitFromCube);
    ...
    blitPipelineCreateInfo.attachment_info.color_attachment_count = 1;
    ...
    blitPipelineCreateInfo.blend_constants[0] = 1.0f;
    blitPipelineCreateInfo.blend_constants[1] = 1.0f;
    blitPipelineCreateInfo.blend_constants[2] = 1.0f;
    blitPipelineCreateInfo.blend_constants[3] = 1.0f;
