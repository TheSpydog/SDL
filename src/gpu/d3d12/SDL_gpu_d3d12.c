...
struct D3D12Texture
{
    ...
    Uint32 subresourceCount; /* layerCount * level_count */
...
static void D3D12_WaitForFences(SDL_GPURenderer *driverData, bool wait_all, SDL_GPUFence *const *fences, Uint32 fence_count);
    ...
    D3D12Shader *fragment_shader)
{
    ...
    if (vertex_shader->sampler_count > 0) {
        // Vertex Samplers
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER;
        descriptorRange.NumDescriptors = vertex_shader->sampler_count;
    ...
    if (vertex_shader->storage_texture_count) {
        // Vertex storage textures
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = vertex_shader->storage_texture_count;
        descriptorRange.BaseShaderRegister = vertex_shader->sampler_count;
        descriptorRange.RegisterSpace = 0;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    if (vertex_shader->storage_buffer_count) {

        // Vertex storage buffers
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = vertex_shader->storage_buffer_count;
        descriptorRange.BaseShaderRegister = vertex_shader->sampler_count + vertex_shader->storage_texture_count;
        descriptorRange.RegisterSpace = 0;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    // Vertex Uniforms
    for (Uint32 i = 0; i < vertex_shader->uniform_buffer_count; i += 1) {
    ...
    if (fragment_shader->sampler_count) {
        // Fragment Samplers
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER;
        descriptorRange.NumDescriptors = fragment_shader->sampler_count;
        descriptorRange.BaseShaderRegister = 0;
    ...
    if (fragment_shader->storage_texture_count) {
        // Fragment Storage Textures
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = fragment_shader->storage_texture_count;
        descriptorRange.BaseShaderRegister = fragment_shader->sampler_count;
        descriptorRange.RegisterSpace = 2;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    if (fragment_shader->storage_buffer_count) {
        // Fragment Storage Buffers
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = fragment_shader->storage_buffer_count;
        descriptorRange.BaseShaderRegister = fragment_shader->sampler_count + fragment_shader->storage_texture_count;
        descriptorRange.RegisterSpace = 2;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    // Fragment Uniforms
    for (Uint32 i = 0; i < fragment_shader->uniform_buffer_count; i += 1) {
        rootParameter.ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
        rootParameter.Descriptor.ShaderRegister = i;
    ...
    const SDL_GPUComputePipelineCreateInfo *createInfo)
{
    ...
    if (createInfo->readonly_storage_texture_count) {
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = createInfo->readonly_storage_texture_count;
        descriptorRange.BaseShaderRegister = 0;
        descriptorRange.RegisterSpace = 0;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    if (createInfo->readonly_storage_buffer_count) {
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
        descriptorRange.NumDescriptors = createInfo->readonly_storage_buffer_count;
        descriptorRange.BaseShaderRegister = createInfo->readonly_storage_texture_count;
        descriptorRange.RegisterSpace = 0;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    if (createInfo->writeonly_storage_texture_count) {
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
        descriptorRange.NumDescriptors = createInfo->writeonly_storage_texture_count;
        descriptorRange.BaseShaderRegister = 0;
    ...
    if (createInfo->writeonly_storage_buffer_count) {
        descriptorRange.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
        descriptorRange.NumDescriptors = createInfo->writeonly_storage_buffer_count;
        descriptorRange.BaseShaderRegister = createInfo->writeonly_storage_texture_count;
        descriptorRange.RegisterSpace = 1;
        descriptorRange.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
        descriptorRanges[rangeCount] = descriptorRange;
    ...
    for (Uint32 i = 0; i < createInfo->uniform_buffer_count; i += 1) {
        rootParameter.ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
        rootParameter.Descriptor.ShaderRegister = i;
    ...
    const SDL_GPUComputePipelineCreateInfo *createinfo)
{
    ...
    compute_pipeline->readonly_storage_texture_count = createinfo->readonly_storage_texture_count;
    compute_pipeline->readonly_storage_buffer_count = createinfo->readonly_storage_buffer_count;
    compute_pipeline->writeonly_storage_texture_count = createinfo->writeonly_storage_texture_count;
    compute_pipeline->writeonly_storage_buffer_count = createinfo->writeonly_storage_buffer_count;
    compute_pipeline->uniform_buffer_count = createinfo->uniform_buffer_count;
...
static bool D3D12_INTERNAL_ConvertRasterizerState(SDL_GPURasterizerState rasterizer_state, D3D12_RASTERIZER_DESC *desc)
{
    ...
    switch (rasterizer_state.front_face) {
    ...
    if (rasterizer_state.depth_bias_enable) {
    ...
    D3D12_BLEND_DESC *blendDesc)
{
    ...
    for (UINT i = 0; i < MAX_COLOR_TARGET_BINDINGS; i += 1) {
        ...
        if (i < pipelineInfo->attachment_info.color_attachment_count) {

            SDL_GPUColorAttachmentBlendState sdlBlendState = pipelineInfo->attachment_info.color_attachment_descriptions[i].blend_state;

            rtBlendDesc.BlendEnable = sdlBlendState.blend_enable;
            rtBlendDesc.SrcBlend = SDLToD3D12_BlendFactor[sdlBlendState.src_color_blend_factor];
            rtBlendDesc.DestBlend = SDLToD3D12_BlendFactor[sdlBlendState.dst_color_blend_factor];
            rtBlendDesc.BlendOp = SDLToD3D12_BlendOp[sdlBlendState.color_blend_op];
            rtBlendDesc.SrcBlendAlpha = SDLToD3D12_BlendFactorAlpha[sdlBlendState.src_alpha_blend_factor];
            rtBlendDesc.DestBlendAlpha = SDLToD3D12_BlendFactorAlpha[sdlBlendState.dst_alpha_blend_factor];
...
static bool D3D12_INTERNAL_ConvertDepthStencilState(SDL_GPUDepthStencilState depth_stencil_state, D3D12_DEPTH_STENCIL_DESC *desc)
{
    ...
    desc->DepthEnable = depth_stencil_state.depth_test_enable == true ? TRUE : FALSE;
    desc->DepthWriteMask = depth_stencil_state.depth_write_enable == true ? D3D12_DEPTH_WRITE_MASK_ALL : D3D12_DEPTH_WRITE_MASK_ZERO;
    desc->DepthFunc = SDLToD3D12_CompareOp[depth_stencil_state.compare_op];
    desc->StencilEnable = depth_stencil_state.stencil_test_enable == true ? TRUE : FALSE;
...
{
    if (desc == NULL || vertex_input_state.vertex_attribute_count == 0) {
        return false;
    }

    for (Uint32 i = 0; i < vertex_input_state.vertex_attribute_count; i += 1) {
    ...
    const SDL_GPUGraphicsPipelineCreateInfo *createinfo)
{
    ...
    if (createinfo->vertex_input_state.vertex_attribute_count > 0) {
        psoDesc.InputLayout.pInputElementDescs = inputElementDescs;
        psoDesc.InputLayout.NumElements = createinfo->vertex_input_state.vertex_attribute_count;
    ...
    psoDesc.NumRenderTargets = createinfo->attachment_info.color_attachment_count;
    for (uint32_t i = 0; i < createinfo->attachment_info.color_attachment_count; i += 1) {
    ...
    for (Uint32 i = 0; i < createinfo->vertex_input_state.vertex_binding_count; i += 1) {
        pipeline->vertexStrides[i] = createinfo->vertex_input_state.vertex_bindings[i].stride;
    }

    pipeline->primitive_type = createinfo->primitive_type;
    pipeline->blend_constants[0] = createinfo->blend_constants[0];
    pipeline->blend_constants[1] = createinfo->blend_constants[1];
    pipeline->blend_constants[2] = createinfo->blend_constants[2];
    pipeline->blend_constants[3] = createinfo->blend_constants[3];
    pipeline->stencilRef = createinfo->depth_stencil_state.reference;

    pipeline->vertexSamplerCount = vertShader->sampler_count;
    pipeline->vertexStorageTextureCount = vertShader->storage_texture_count;
    pipeline->vertexStorageBufferCount = vertShader->storage_buffer_count;
    pipeline->vertexUniformBufferCount = vertShader->uniform_buffer_count;

    pipeline->fragmentSamplerCount = fragShader->sampler_count;
    pipeline->fragmentStorageTextureCount = fragShader->storage_texture_count;
    pipeline->fragmentStorageBufferCount = fragShader->storage_buffer_count;
    pipeline->fragmentUniformBufferCount = fragShader->uniform_buffer_count;
    ...
    const SDL_GPUSamplerCreateInfo *createinfo)
{
    ...
    samplerDesc.Filter = SDLToD3D12_Filter(
        ...
        createinfo->compare_enable,
        createinfo->anisotropy_enable);
    ...
    const SDL_GPUShaderCreateInfo *createinfo)
{
    ...
    shader->sampler_count = createinfo->sampler_count;
    shader->storage_buffer_count = createinfo->storage_buffer_count;
    shader->storage_texture_count = createinfo->storage_texture_count;
    shader->uniform_buffer_count = createinfo->uniform_buffer_count;
    ...
    bool isSwapchainTexture)
{
    ...
    if (createinfo->type != SDL_GPU_TEXTURETYPE_3D) {
        ...
        desc.MipLevels = (UINT16)createinfo->level_count;
    ...
    if ((createinfo->usage_flags & SDL_GPU_TEXTUREUSAGE_SAMPLER) ||
        ...
        if (createinfo->type == SDL_GPU_TEXTURETYPE_CUBE) {
            ...
            srvDesc.TextureCube.MipLevels = createinfo->level_count;
            srvDesc.TextureCube.MostDetailedMip = 0;
            srvDesc.TextureCube.ResourceMinLODClamp = 0;
        } else if (createinfo->type == SDL_GPU_TEXTURETYPE_2D_ARRAY) {
            srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
            srvDesc.Texture2DArray.MipLevels = createinfo->level_count;
            ...
            srvDesc.Texture3D.MipLevels = createinfo->level_count;
            srvDesc.Texture3D.MostDetailedMip = 0;
            srvDesc.Texture3D.ResourceMinLODClamp = 0; // default behavior
        } else {
            srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
            srvDesc.Texture2D.MipLevels = createinfo->level_count;
    ...
    texture->subresourceCount = createinfo->level_count * layerCount;
    ...
    for (Uint32 layerIndex = 0; layerIndex < layerCount; layerIndex += 1) {
        for (Uint32 levelIndex = 0; levelIndex < createinfo->level_count; levelIndex += 1) {
    ...
    Uint32 level)
{
    Uint32 index = D3D12_INTERNAL_CalcSubresource(
        level,
        layer,
        container->header.info.level_count);
    ...
    const SDL_GPUDepthStencilAttachmentInfo *depth_stencil_attachment_info)
{
    ...
    for (Uint32 i = 0; i < color_attachment_count; i += 1) {
    ...
    d3d12CommandBuffer->colorAttachmentTextureSubresourceCount = color_attachment_count;
    ...
    SDL_GPUGraphicsPipeline *graphics_pipeline)
{
    ...
    float blendFactor[4] = {
        pipeline->blend_constants[0],
        pipeline->blend_constants[1],
        pipeline->blend_constants[2],
        pipeline->blend_constants[3]
    };
    ...
    Uint32 binding_count)
{
    ...
    d3d12CommandBuffer->vertexBufferCount =
        SDL_max(d3d12CommandBuffer->vertexBufferCount, first_binding + binding_count);
    ...
    Uint32 storage_buffer_binding_count)
{
    ...
    d3d12CommandBuffer->computeWriteOnlyStorageTextureSubresourceCount = storage_texture_binding_count;
    d3d12CommandBuffer->computeWriteOnlyStorageBufferCount = storage_buffer_binding_count;
     ...
     */
    if (storage_texture_binding_count > 0) {
        for (Uint32 i = 0; i < storage_texture_binding_count; i += 1) {
    ...
    if (storage_buffer_binding_count > 0) {
        for (Uint32 i = 0; i < storage_buffer_binding_count; i += 1) {
    ...
    SDL_GPUComputePipeline *compute_pipeline)
{
    ...
    for (Uint32 i = 0; i < pipeline->uniform_buffer_count; i += 1) {
    ...
    if (pipeline->writeonly_storage_texture_count > 0) {
        for (Uint32 i = 0; i < pipeline->writeonly_storage_texture_count; i += 1) {
    ...
    if (pipeline->writeonly_storage_buffer_count > 0) {
        for (Uint32 i = 0; i < pipeline->writeonly_storage_buffer_count; i += 1) {
    ...
    D3D12CommandBuffer *command_buffer)
{
    ...
    if (command_buffer->needComputeReadOnlyStorageTextureBind) {
        if (compute_pipeline->readonly_storage_texture_count > 0) {
            for (Uint32 i = 0; i < compute_pipeline->readonly_storage_texture_count; i += 1) {
            ...
            D3D12_INTERNAL_WriteGPUDescriptors(
                ...
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
                cpuHandles,
                compute_pipeline->readonly_storage_texture_count,
                &gpuDescriptorHandle);
    ...
    if (command_buffer->needComputeReadOnlyStorageBufferBind) {
        if (compute_pipeline->readonly_storage_buffer_count > 0) {
            for (Uint32 i = 0; i < compute_pipeline->readonly_storage_buffer_count; i += 1) {
            ...
            D3D12_INTERNAL_WriteGPUDescriptors(
                ...
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
                cpuHandles,
                compute_pipeline->readonly_storage_buffer_count,
                &gpuDescriptorHandle);
    ...
    for (Uint32 i = 0; i < MAX_UNIFORM_BUFFERS_PER_STAGE; i += 1) {
        if (command_buffer->needComputeUniformBufferBind[i]) {
            if (compute_pipeline->uniform_buffer_count > i) {
    ...
    bool cycle)
{
    ...
    Uint32 pixelsPerRow = source->image_pitch;
    Uint32 rowPitch;
    Uint32 alignedRowPitch;
    Uint32 rowsPerSlice = source->image_height;
    ...
    const SDL_GPUTextureTransferInfo *destination)
{
    ...
    D3D12_TEXTURE_COPY_LOCATION sourceLocation;
    D3D12_TEXTURE_COPY_LOCATION destinationLocation;
    Uint32 pixelsPerRow = destination->image_pitch;
    Uint32 rowPitch;
    Uint32 alignedRowPitch;
    Uint32 rowsPerSlice = destination->image_height;
    bool needsRealignment;
    bool needsPlacementCopy;
    ...
    SDL_GPUTexture *texture)
{
    ...
    for (Uint32 layerOrDepthIndex = 0; layerOrDepthIndex < container->header.info.layer_count_or_depth; layerOrDepthIndex += 1) {
        for (Uint32 levelIndex = 1; levelIndex < container->header.info.level_count; levelIndex += 1) {
    ...
    D3D12TextureContainer *pTextureContainer)
{
    ...
    pTextureContainer->header.info.level_count = 1;
    ...
    Uint32 *height)
{
    ...
    *width = windowData->textureContainers[swapchainIndex].header.info.width;
    *height = windowData->textureContainers[swapchainIndex].header.info.height;
    ...
    Uint32 fence_count)
{
    D3D12Renderer *renderer = (D3D12Renderer *)driverData;
    D3D12Fence *fence;
    HANDLE *events = SDL_stack_alloc(HANDLE, fence_count);
    HRESULT res;
    ...
    D3D12Renderer *renderer)
{
    ...
    SDL_zero(createinfo);
    createinfo.code = (Uint8 *)D3D12_FullscreenVert;
    createinfo.code_size = sizeof(D3D12_FullscreenVert);
    ...
    createinfo.code = (Uint8 *)D3D12_BlitFrom2D;
    createinfo.code_size = sizeof(D3D12_BlitFrom2D);
    createinfo.stage = SDL_GPU_SHADERSTAGE_FRAGMENT;
    createinfo.sampler_count = 1;
    createinfo.uniform_buffer_count = 1;
    ...
    createinfo.code = (Uint8 *)D3D12_BlitFrom2DArray;
    createinfo.code_size = sizeof(D3D12_BlitFrom2DArray);
    ...
    createinfo.code = (Uint8 *)D3D12_BlitFrom3D;
    createinfo.code_size = sizeof(D3D12_BlitFrom3D);
    ...
    createinfo.code = (Uint8 *)D3D12_BlitFromCube;
    createinfo.code_size = sizeof(D3D12_BlitFromCube);
    ...
    createinfo.anisotropy_enable = 0;
    createinfo.compare_enable = 0;
