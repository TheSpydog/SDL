    ...
    Uint32 *blitPipelineCapacity)
{
    ...
    textureSamplerBinding.sampler =
        filter_mode == SDL_GPU_FILTER_NEAREST ? blitNearestSampler : blitLinearSampler;
    ...
    const SDL_GPUComputePipelineCreateInfo *createinfo)
{
    ...
    if (device->debug_mode) {
        ...
        if (createinfo->writeonly_storage_texture_count > MAX_COMPUTE_WRITE_TEXTURES) {
            SDL_assert_release(!"Compute pipeline write-only texture count cannot be higher than 8!");
            return NULL;
        }
        if (createinfo->writeonly_storage_buffer_count > MAX_COMPUTE_WRITE_BUFFERS) {
            SDL_assert_release(!"Compute pipeline write-only buffer count cannot be higher than 8!");
            return NULL;
        }
        if (createinfo->thread_count_x == 0 ||
            createinfo->thread_count_y == 0 ||
            createinfo->thread_count_z == 0) {
    ...
    const SDL_GPUGraphicsPipelineCreateInfo *graphicsPipelineCreateInfo)
{
    ...
    if (device->debug_mode) {
        for (Uint32 i = 0; i < graphicsPipelineCreateInfo->attachment_info.color_attachment_count; i += 1) {
    ...
    const SDL_GPUTextureCreateInfo *createinfo)
{
    ...
    if (device->debug_mode) {
        ...
        if (createinfo->level_count <= 0) {
            SDL_assert_release(!"For any texture: level_count must be >= 1");
            failed = true;
        }
        ...
        if (createinfo->type == SDL_GPU_TEXTURETYPE_CUBE) {
            ...
            if (createinfo->type == SDL_GPU_TEXTURETYPE_2D_ARRAY) {
                ...
                if (createinfo->sample_count > SDL_GPU_SAMPLECOUNT_1 && createinfo->level_count > 1) {
                    SDL_assert_release(!"For 2D textures: if sample_count is >= SDL_GPU_SAMPLECOUNT_1, then level_count must be 1");
                    failed = true;
                }
            }
    ...
    const SDL_GPUDepthStencilAttachmentInfo *depth_stencil_attachment_info)
{
    ...
    if (color_attachment_infos == NULL && color_attachment_count > 0) {
        SDL_InvalidParamError("color_attachment_infos");
        return NULL;
    }

    if (color_attachment_count > MAX_COLOR_TARGET_BINDINGS) {
        SDL_SetError("color_attachment_count exceeds MAX_COLOR_TARGET_BINDINGS");
        return NULL;
    }
    ...
    Uint32 binding_count)
{
    ...
    if (texture_sampler_bindings == NULL && binding_count > 0) {
    ...
    Uint32 binding_count)
{
    ...
    if (storage_textures == NULL && binding_count > 0) {
    ...
    Uint32 binding_count)
{
    ...
    if (storage_buffers == NULL && binding_count > 0) {
    ...
    Uint32 storage_buffer_binding_count)
{
    ...
    if (storage_texture_bindings == NULL && storage_texture_binding_count > 0) {
        SDL_InvalidParamError("storage_texture_bindings");
        return NULL;
    }
    if (storage_buffer_bindings == NULL && storage_buffer_binding_count > 0) {
        SDL_InvalidParamError("storage_buffer_bindings");
        return NULL;
    }
    if (storage_texture_binding_count > MAX_COMPUTE_WRITE_TEXTURES) {
        SDL_InvalidParamError("storage_texture_binding_count");
        return NULL;
    }
    if (storage_buffer_binding_count > MAX_COMPUTE_WRITE_BUFFERS) {
        SDL_InvalidParamError("storage_buffer_binding_count");
        return NULL;
    }
    if (COMMAND_BUFFER_DEVICE->debug_mode) {
        CHECK_COMMAND_BUFFER_RETURN_NULL
    ...
    SDL_GPUTexture *texture)
{
    ...
    if (COMMAND_BUFFER_DEVICE->debug_mode) {
        ...
        if (header->info.level_count <= 1) {
            SDL_assert_release(!"Cannot generate mipmaps for texture with level_count <= 1!");
            return;
        }
